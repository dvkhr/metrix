...
//     Если возникает ошибка при получении метрик, она возвращается вызывающей стороне.
//  2. Метрики сериализуются в JSON с отступами для удобства чтения.
//     Если сериализация завершается ошибкой, она возвращается вызывающей стороне.
//  3. Сериализованные данные записываются в writer.
//     Если запись завершается ошибкой, она возвращается вызывающей стороне.
//
// Параметры:
...
//
// Логика работы:
//  1. Считывает все данные из reader в байтовый массив.
//     Если чтение завершается ошибкой, она возвращается вызывающей стороне.
//  2. Вызывает метод List хранилища метрик для получения текущего состояния хранилища.
//     Если возникает ошибка при получении состояния, она возвращается вызывающей стороне.
//  3. Десериализует JSON-данные в текущее состояние хранилища.
//     Если десериализация завершается ошибкой, она возвращается вызывающей стороне.
//
// Параметры:
// - ms: Интерфейс MetricStorage, предоставляющий доступ к хранилищу метрик.
...
// преобразует её в метрики типа "gauge" или "counter" и отправляет в указанный канал.
//
// Логика работы:
...
//
// Функция используется для мониторинга производительности приложения,
// включая использование памяти, сборку мусора и другие аспекты работы runtime.
//
// Логика работы:
//  1. Определяется вспомогательная функция collectMetric, которая создает объект Metrics
//     на основе типа метрики (GaugeMetric или CounterMetric) и отправляет его в канал.
